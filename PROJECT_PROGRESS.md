...
**Current phase:** Supplier archive-dialog hotfix — v1.1.4
...
| Show/hide sidebar + collapsible menus | `PENDING - NEXT VERSION` | Keep the current detailed full-text sidebar and visual style. Clicking the application logo toggles the whole sidebar: open shows the complete text menu, hidden leaves only a narrow logo strip and expands the content area. While open, each parent menu group can independently collapse/expand its submenu; keep the active group open and restore its state when the sidebar returns. Do not add an icon-only menu. Preserve the user's sidebar and menu-group preferences. Bundle this with the next application version instead of publishing a dedicated UI-only release. |
...
| v1.1.5 | Booking template, preview, and Gmail send | `PENDING` | System generates versioned SOP-based subject/body/attachments, requires recipient and content preview, sends through the connected user, and stores Gmail thread/message IDs plus the exact sent snapshot. |
| v1.1.5 | Revision impact and booking amendment | `PENDING` | System compares the new Reservation publication with the Vendor working version and requires a per-item decision: unchanged, add, change, rebook, or cancel; confirmed bookings are never silently overwritten. |
| v1.1.5 | Vendor re-check and booking completion | `PENDING` | Every supplier booking is marked Sent, Pending, Confirmed, Changed, or Canceled with actor/time/note/evidence; incomplete items block Booking Complete publication and the resulting notification reaches Reservation. |
...
| 2026-07-28 | Final sidebar navigation decision: retain the detailed full-text menu; clicking the application logo shows/hides the entire sidebar, while each parent menu can separately collapse/expand its submenu when the sidebar is open. Hidden mode leaves only a narrow logo strip and expands the content area. Reopening restores the active submenu and saved menu-group state. Icon-only navigation remains out of scope. Remind the owner and bundle this work with the next application version rather than issuing a dedicated release. | `PENDING - NEXT VERSION` |
