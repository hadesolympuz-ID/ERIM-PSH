...
| Show/hide text sidebar | `PENDING - NEXT VERSION` | Keep the current detailed full-text sidebar and visual style. Clicking the application logo toggles the whole sidebar: open shows the complete text menu, hidden leaves only a narrow logo strip and expands the content area. Do not add an icon-only menu. Preserve the active submenu and the user's open/hidden preference. Bundle this with the next application version instead of publishing a dedicated UI-only release. |
...
| 2026-07-28 | Sidebar navigation decision updated: retain the detailed full-text menu, but allow the entire sidebar to show/hide by clicking the application logo. Hidden mode leaves only a narrow logo strip and expands the content area; reopening restores the active submenu. Icon-only navigation remains out of scope. Remind the owner and bundle this work with the next application version rather than issuing a dedicated release. | `PENDING - NEXT VERSION` |
