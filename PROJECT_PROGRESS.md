...
| Collapsible sidebar submenus | `PENDING` | Parent menus such as Reservation, Vendor Booking, and Manager/Admin can open/close their text submenus. Keep the current detailed text-first sidebar; do not add icon-only or fully collapsed sidebar mode. Prefer one submenu group open at a time, keep the active group open, and retain vertical scrolling as the final overflow fallback. |
...
| 2026-07-28 | Sidebar navigation improvement recorded for a later UI update: text-based submenu groups will become collapsible/expandable so growing menus remain manageable. The current detailed labels and visual style are retained; icon-only sidebar mode is explicitly out of scope. | `PENDING` |
