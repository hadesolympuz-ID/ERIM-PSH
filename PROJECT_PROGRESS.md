...
**Current phase:** Vendor Booking v1.1.18 GC-02 safety patch released; controlled Gmail/Drive/operator UAT remains
...
| 2026-07-30 | Owner revised the Generate layout again before implementation. The main preparation workspace becomes: left Daywise Vendor-service tree with sticky `Generate Booking`; middle exact Supplier Master/SOP/channel/recipient detail for the current service; right exact Product/Contract/Rate detail with focused correction and non-blocking Pending Rate handling. Generate validates and regroups selected stable services, then opens a full-screen communication popup. Its left panel lists the outgoing supplier packages sorted by SOP/channel then Supplier, Customer Code, and Day order; middle shows the booking message and, for Email, sender plus TO/CC/BCC; right contains the explicit Gmail Send or applicable external-channel action. `Previous`, `Package X of N`, and `Next pending` support large batches without automatic sending; successful Send remains visible with Open Gmail Thread and Next pending. The latest design is recorded in Section 11 of `docs/VENDOR_V119_UAT_FOLLOWUP_NOTES.md`; no application source was changed. | `LATEST DESIGN — AWAITING IMPLEMENTATION` |
| 2026-07-30 | Vendor Booking v1.1.10 follow-up implementation completed locally. Day Start/Finish Time no longer blocks Split or online posting; Add Service inserts and focuses the blank row at the top; optional Client Tag feeds the standardized booking Subject; Vendor Generate excludes every non-`VENDOR` Type in the backend and UI. The preparation workspace now has Daywise selection, exact Supplier/SOP/single-channel detail, and exact Product/Contract/Rate detail with focused correction plus non-blocking Pending Rate skip. Generate creates supplier-package snapshots and opens a full-screen queue/message/channel workspace with SOP/channel sorting, progress totals, direct package selection, Previous/Next pending, unsaved-change handling, connected Gmail Send, external-channel evidence, and preserved result visibility. Intentional resend requires a prior proven attempt, Continue/Cancel, reviewed replacement recipients, reason, final confirmation, a new linked idempotent ledger attempt, and separate Gmail thread history; content changes remain Amendment. Apps Script source accepts optional Day times, stores Client Tag, and records resend linkage/reason. All 53 automated tests and source checks pass; packaging, smoke, live Apps Script deployment, and connected-account Gmail UAT remain. | `IMPLEMENTED — LOCAL UAT` |
| 2026-07-30 | Vendor Booking v1.1.10 Windows UAT candidate packaged successfully. All 53 automated tests, desktop and Apps Script syntax checks, diff hygiene, and secret scan pass. The NSIS installer reports Product Version `1.1.10`; the unpacked executable reports `1.1.10.0`. Packaged ASAR inspection confirms the full-screen communication workspace, Next pending navigation, Client Tag support, and linked resend ledger. The isolated packaged app remained alive through the eight-second smoke gate. Installer size: `111,066,846` bytes. SHA-256: `2DC3A6A9C25A9F3FB638D7E14E4207B2B83400D9FC1C61AEFB6335A2506165EB`. Live Apps Script deployment, GitHub release, connected-account Gmail send/readback, and full operational UAT remain. | `UAT BUILD READY` |
| 2026-07-30 | Vendor Booking v1.1.11 hotfix started after UAT exposed that closing the communication popup left a generated-but-unsent service disabled in the Daywise tree, making it look processed. `GENERATED` is now labeled `DRAFT READY — NOT SENT`; each draft has a Resume action, the preparation toolbar can restore every generated draft into its SOP-sorted full-screen batch after close or app restart, and Escape uses the same unsaved-change guard as the explicit Close button. Only successful delivery may create a Sent state. | `IMPLEMENTED — LOCAL VERIFICATION` |
| 2026-07-30 | Vendor Booking v1.1.11 generated-draft recovery hotfix packaged successfully. All 53 automated tests, syntax checks, diff hygiene, and secret scan pass. Packaged ASAR confirms Resume Draft recovery, the explicit Not Sent label, Escape close guard, and version `1.1.11`; the executable reports Product Version `1.1.11.0` and remained alive through the isolated eight-second smoke gate. Installer size: `111,067,207` bytes. SHA-256: `2573AAD37506F7E957D121403C415B4965BAD1FFCD994E2D2095B5AD351CC0D3`. | `UAT BUILD READY` |
| 2026-07-30 | Vendor Booking v1.1.12 Priority 1–5 implementation, Windows packaging, and backend deployment completed. Deterministic Email/WhatsApp/Portal/Other package and service sorting, exact Portal transaction Service IDs, the configured Eka Jaya 14/15/16-day Deposit/Prepaid rule, Generate and pre-Send Gmail profile checks, derived Email evidence marks, Delivery History entry points, attempt-aware resend/reconciliation/reply tracking, authoritative no-TO ledger protection, and local Supplier Rate maker-checker approval/sync controls are included. `STANDARD_V1` now prints Day header, optional Start/Finish, and Hotel; automatic attachments remain disabled pending an approved rule. All 57 automated tests, desktop and Apps Script syntax checks, and diff hygiene pass. Packaged ASAR confirms the v1.1.12 controls; Product Version is `1.1.12.0` and the app remained alive through the isolated eight-second smoke gate. Installer size: `111,076,232` bytes. SHA-256: `FBC572CC5B4F4D3C61143685AA711D557E9DBF7DD26473D110507F271A8E879E`. Apps Script Web App Version 21 is active on the existing deployment URL and its health endpoint returns HTTP 200 with `ok: true`. Git push/release, connected-account Gmail, and cross-account approval UAT remain. | `BACKEND LIVE — RELEASE READY` |
| 2026-07-30 | GitHub Release v1.1.12 published as Latest from implementation commit `4deedb9`. The installer, blockmap, and `latest.yml` are uploaded and visible to the desktop updater; the online installer digest matches SHA-256 `FBC572CC5B4F4D3C61143685AA711D557E9DBF7DD26473D110507F271A8E879E`. Draft PR #1 was updated for the Priority 1–5 scope. Connected-account Gmail send/readback, cross-account Supplier Rate approval, configured Eka Jaya live Portal boundaries, and full operational UAT remain controlled gates. | `RELEASED — LIVE UAT` |
| 2026-07-30 | An initial consolidated v1.1.13 New Itinerary, Generate tree, and Cancel Generate follow-up was captured in Section 14 of `docs/VENDOR_V119_UAT_FOLLOWUP_NOTES.md`. The owner then requested one-workspace-at-a-time confirmation; the next progress row supersedes this consolidated scope for implementation sequencing. | `RECORDED — SUPERSEDED BY PHASE 1` |
| 2026-07-30 | v1.1.13 Phase 1 was narrowed and confirmed as Vendor Booking → New Itinerary only; implementation remains paused. Day Wise Header must come from the matching posted itinerary Program without overwriting later manual edits. Hour-only input normalizes to `HH:00`; the workspace becomes more compact; dates display as `dd/MMMM/yyyy` while storage remains ISO. Day 0 is never automatic: staff adds the single optional pre-arrival record through `+ Add Day 0` below `Day Wise & micro split`; while it exists the add action is unavailable and `Delete Day 0` can remove only that record with stronger confirmation when it contains details or Micro Splits. Rebuild Dates preserves its adjustable date/details while rebuilding Day 1+. Total Pax is a direct manual input; existing Adult/Child/Infant fields remain intact pending a later owner decision. Arrival and departure date/time continue to come automatically from the posted itinerary and remain reviewable/editable. Generate-tree and Cancel Generate follow-ups remain outside Phase 1. | `PHASE 1 CONFIRMED — AWAITING IMPLEMENTATION` |
| 2026-07-30 | v1.1.13 Phase 2 was confirmed specifically for the maximized `Generate & Send Supplier Booking` popup and then corrected by the owner from package-level to service-item-level cancellation. Supplier packages remain grouped, but every generated service child receives its own compact `Cancel generate` action while the package is `DRAFT READY — NOT SENT`. Cancel releases only the selected stable Service ID, preserves all siblings, deterministically rebuilds the remaining Subject/body/Portal membership/snapshot hash, and records `GENERATE_ITEM_CANCELED` without any Send Attempt or delivery evidence. The empty package becomes `GENERATE_CANCELED` only when its final service is removed. | `PHASE 2 FINAL SCOPE — AWAITING IMPLEMENTATION` |
| 2026-07-30 | v1.1.13 Phase 3 was confirmed for the Generate preparation submenu. The Client → Day → Vendor service hierarchy becomes a compact Windows-style directory tree with standard system typography, restrained headings, aligned checkboxes, branch lines, subtle selection, and solid-black Day Wise Header text. The selected item/package's Supplier/SOP panel always exposes Email, WhatsApp, Portal, and Other as single-choice options, defaulting from the stored database/SOP channel. Every option remains selectable and shows `READY` or `DATA MISSING` plus its destination: Email TO/CC, WhatsApp name/number, Portal URL/account reference without passwords, or Other instructions. Selecting incomplete data opens an in-place completion popup containing only the missing channel fields. `Save locally & queue online` stages the exact SOP/Recipient update with audit/base version, refreshes the package to `READY — LOCAL PENDING ONLINE`, and idempotently queues dependency-ordered online publication while preserving offline/conflict states and local provenance. Generate remains blocked only until the selected channel is locally complete. | `PHASE 3 CONFIRMED — AWAITING IMPLEMENTATION` |
| 2026-07-30 | The complete owner-confirmed Vendor Booking v1.1.13 target milestone was compiled into `docs/VENDOR_V113_BASE_PLAN.md`. It is the authoritative development record for Phase 1 New Itinerary, Phase 2 per-service-item Cancel Generate in the maximized communication popup, and Phase 3 compact Generate tree plus always-visible channel completion/local-online queue. Phase 2 explicitly supports returning one incorrect item out of a large generated package to `NOT_GENERATED` while keeping every sibling generated and rebuilding the package snapshot deterministically. The plan locks schema/state changes, delivery order, security/non-goals, acceptance checks, packaging/deployment gates, and controlled live-UAT boundaries. No application source, schema, version, deployment, or release was changed while compiling the plan. | `NEXT MILESTONE READY FOR IMPLEMENTATION` |
| 2026-07-30 | v1.1.13 Phase 4 was confirmed as a service-level Booking Delivery Report. One row represents one stable Service ID and shows Code, Client, service date/Day, Product, Supplier, channel, authoritative booking/delivery status, `dd/MMMM/yyyy HH:mm WITA` Sent time, and only the actions valid for that item. Default `Needs Action`, free-word search (including Product/Day Header terms such as `dinner`), status/channel/Supplier/service-date/Sent-date/evidence/reply filters, deterministic sort choices, expandable delivery evidence, sticky compact 100+ row handling, and preserved report context are required. Package-level actions still resolve every linked service and cannot bypass Send/Resend/Reconcile safeguards. The compiled v1.1.13 base plan was updated to include this fourth confirmed workspace. | `PHASE 4 CONFIRMED — AWAITING IMPLEMENTATION` |
| 2026-07-30 | v1.1.13 Phase 5 was confirmed for the post-Generate experience. The maximized `Generate & Send Supplier Booking` popup must land on a compact Excel-inspired spreadsheet grid with one primary row per supplier package and expandable service child rows carrying per-item Cancel Generate. Standard 11–12px text, sticky headings, horizontal/vertical scrolling, package status/action-needed columns, `Open / Review`, and preserved selection/expansion/scroll are required. Detail mode adds `Back to List`, Previous/Next Package, `Package X of N`, and Next Pending; Resume also returns to the list first. Deterministic Generate order remains authoritative, and canceling one service refreshes only its package membership/count. | `PHASE 5 CONFIRMED — AWAITING IMPLEMENTATION` |
| 2026-07-30 | The compiled v1.1.13 scope was promoted to the authoritative `docs/VENDOR_V113_BASE_PLAN.md` for the next development step. It contains five confirmed phases, 86 acceptance gates, schema/read-model requirements, security and non-goals, implementation order, completion definition, and a kickoff boundary that keeps real sends and representative online Supplier mutations inside controlled UAT. No application implementation was started. | `BASE PLAN READY — IMPLEMENTATION NOT STARTED` |
| 2026-07-30 | Vendor Booking v1.1.13 desktop implementation and Windows packaging completed. New Itinerary now extracts posted Program headings, supports one explicit preserved Day 0, independent manual Total Pax, hour-only `HH:00` normalization, compact typography, and `dd/MMMM/yyyy` display over ISO storage. Generate preparation exposes a compact Client → Day → service tree and all four single-choice channels with readiness/destination labels plus local Supplier Master completion queued for approved publication. Generate opens on a spreadsheet-style batch grid, keeps Back/List/Previous/Next controls, and permits auditable `GENERATE_ITEM_CANCELED` only per unsent stable Service ID while rebuilding sibling snapshots/hashes. Vendor Inbox now includes a service-level Booking Delivery Report with Needs Action, word/status/channel/Supplier/date/evidence filters, WITA sent time, and contextual history/Gmail/sync actions. All 60 tests, syntax checks, diff hygiene, ASAR inspection, and the isolated eight-second smoke gate pass. Installer size: `111,081,693` bytes. SHA-256: `39BED6D550044C933F09E53D84182220C5CC607CF979800E2834F080A14C4920`. Apps Script source includes Day 0 and manual Total Pax support, but this workstation has no authenticated Apps Script deployment binding; Web App v21 remains live until the source is deployed and health-checked. No real supplier message or representative Supplier online mutation was used. | `DESKTOP RELEASE READY — BACKEND DEPLOYMENT GATE` |
| 2026-07-30 | GitHub Release `v1.1.13` was published as Latest from implementation commit `e9f64dd`. Installer, blockmap, and `latest.yml` are uploaded; the online installer size and SHA-256 exactly match the verified local artifact (`111,081,693` bytes; `39BED6D550044C933F09E53D84182220C5CC607CF979800E2834F080A14C4920`). Draft PR #1 now describes the full v1.1.13 scope. Desktop updater distribution is live; Apps Script deployment and health verification remain the explicit backend gate before Day 0/manual Total Pax may be called centrally live. | `DESKTOP RELEASED — APPS SCRIPT GATE OPEN` |
| 2026-07-30 | Owner trial of installed v1.1.13 identified four follow-up corrections, recorded in Section 12 of `docs/VENDOR_V113_BASE_PLAN.md`. Pax entry means Adult/Child/Infant remain manually entered; there must not be a separate editable independent Total Pax, and any Total display is derived read-only. The primary date presentation itself must show `dd/MMMM/yyyy`; a helper below a native locale-formatted date input is insufficient. `Rebuild dates` must repopulate matching available Day Wise Headers from the posted itinerary Program while keeping headers editable and protecting later intentional manual edits. Vendor Booking Daily Control Dashboard must gain an Online Process Report driven by real local-save/queue/post results, separating `LOCAL_SAVED`, queued/posting/pending, online-posted, failed, and conflict states with valid retry/review/open actions. This is a correction record only; no application implementation or new version was started. | `OWNER CORRECTION RECORDED — AWAITING IMPLEMENTATION` |
| 2026-07-30 | Owner clarified the generated-item return flow in Section 12.5 of `docs/VENDOR_V113_BASE_PLAN.md`. `Back to list` is navigation only, belongs near the selected Service-item work area, clears stale Item A selection/detail context, and never changes Generated/delivery state. `Cancel Generate item` is the separate confirmed business action: after authoritative success the exact stable Service ID is immediately removed from the active generated batch, returned to `NOT_GENERATED`, siblings and package snapshot are refreshed, and the empty package disappears only after its final Service is removed. Reopening/restarting must reproduce the refreshed SQLite-backed list; a failed cancellation leaves the item visible and cannot show false success. Ten explicit acceptance checks and an A/B example were recorded to prevent another interpretation mismatch. No application implementation was started. | `GENERATED ITEM RETURN FLOW RECORDED — AWAITING IMPLEMENTATION` |
| 2026-07-30 | Vendor Booking v1.1.14 correction implementation and Windows package verification completed. The editable Total Pax field was removed and the compatibility value is now always derived from Adult + Child + Infant. All staff-facing Vendor intake dates now use `dd/MMMM/yyyy` while SQLite/online payloads stay ISO. Rebuild Dates fetches the latest posted itinerary Program and fills blank matching Day Wise Headers without overwriting non-empty manual edits. STANDARD_V1 supplier messages exclude the broad Day Wise Header and include only the exact selected Product/Service plus its date, hotel, and operating times. Back to List moved into the selected-item workspace, clears stale detail context, and Cancel Generate remains a separate per-Service-ID action. Successful Gmail/external delivery immediately delists the package from the active Generated queue; history stays in the Delivery Report/Register. Daily Control now contains a persisted Online Process Report for local save, Micro Split autosave, posting, posted, and failed outcomes. All 60 automated tests, syntax checks, diff hygiene, ASAR source verification, and an isolated eight-second packaged-app smoke gate pass. Installer size: `111,083,314` bytes. SHA-256: `EA56ED59041F992903E19EF6D27F9665F27B1540D0754F993A37D06F6DF74A17`. Apps Script source changed but is not yet deployed. | `V1.1.14 DESKTOP PACKAGE VERIFIED — RELEASE PENDING` |
| 2026-07-30 | GitHub Release `v1.1.14` was published as a non-draft, non-prerelease Latest release from branch `agent/desktop-v1`, implementation commit `0632efb`. Installer, blockmap, and `latest.yml` are uploaded and verified through GitHub. The online installer is exactly `111,083,314` bytes and GitHub reports the matching SHA-256 `EA56ED59041F992903E19EF6D27F9665F27B1540D0754F993A37D06F6DF74A17`. Desktop updater distribution is live. Apps Script deployment remains separate and pending because this workstation has no authenticated deployment binding; no real supplier communication or representative online Vendor mutation was performed. | `V1.1.14 DESKTOP RELEASED — APPS SCRIPT/UAT GATE OPEN` |
| 2026-07-31 | Owner trial found a v1.1.14 communication-popup state bug: after Back to List the active index became `-1` while stale form values and an author CSS grid kept the hidden detail panel active, producing `Package 0 of N` and the same false Unsaved warning from Back, Previous, Next, Close, and window-close paths. The v1.1.15 hotfix limits dirty checking to a genuinely open selected detail, clears stale form/detail context on list return, forces hidden list/work panels out of layout, disables navigation without a selected package, and replaces `Package 0` with `No package selected`. The owner also renamed/simplified the pre-send rollback to `Cancel sending item`: one confirmation returns only the exact stable Service ID to `NOT_GENERATED` using an automatic audit reason. Backend protections remain unchanged—only `GENERATED`, not-sent items with no delivery attempt/evidence can be canceled. All 61 tests, syntax checks, diff hygiene, ASAR checks, and the isolated eight-second packaged-app smoke gate pass. Installer size: `111,083,225` bytes. SHA-256: `30E0F1639434C2253120963D4AAADF50DFEE7EF34516ABA415E0ADF1671E6E1C`. | `V1.1.15 HOTFIX PACKAGE VERIFIED — RELEASE PENDING` |
| 2026-07-31 | GitHub Release `v1.1.15` was published as Latest from implementation commit `d7486a3`. Installer, blockmap, and `latest.yml` are uploaded; GitHub reports the exact verified installer size (`111,083,225` bytes) and matching SHA-256 `30E0F1639434C2253120963D4AAADF50DFEE7EF34516ABA415E0ADF1671E6E1C`. Desktop updater distribution is live for owner retest of Back/List/Previous/Next/Close/window-close and the simplified per-item Cancel sending flow. Apps Script source/deployment is unchanged by this desktop-only hotfix. | `V1.1.15 HOTFIX RELEASED — OWNER RETEST READY` |
| 2026-07-31 | Owner retest on v1.1.15 showed `Header:` still visible in messages that had already been Generated under the legacy template. Source audit confirmed the new generator was correct, but the workspace intentionally preferred the persisted SQLite snapshot and Regenerate reused the displayed legacy body. v1.1.16 adds a controlled compatibility migration whenever a booking is read: only `GENERATED` snapshots with no Send Attempt are scanned; a legacy `- Day ... | Header: ...` segment is removed while the exact Product/Service line is retained, the snapshot hash is recalculated, and `VENDOR_GENERATED_BODY_MIGRATED` is audited. `SENT`, pending-sync, outcome-unknown, or any snapshot with delivery attempts remains immutable. All 62 tests, syntax checks, diff hygiene, ASAR verification, and the isolated eight-second packaged-app smoke gate pass. Installer size: `111,083,432` bytes. SHA-256: `02A9EEBB62FE0286CEDCA2BA2CF29673ADE19AD66244882E153E0F39C0818F28`. | `V1.1.16 LEGACY SNAPSHOT MIGRATION PACKAGE VERIFIED — RELEASE PENDING` |
| 2026-07-31 | GitHub Release `v1.1.16` was published as Latest from implementation commit `f88292d`. Installer, blockmap, and `latest.yml` are uploaded. GitHub reports the exact verified installer size (`111,083,432` bytes) and matching SHA-256 `02A9EEBB62FE0286CEDCA2BA2CF29673ADE19AD66244882E153E0F39C0818F28`. Desktop updater distribution is live; opening each legacy Generated/not-sent package performs the controlled cleanup once. Apps Script source/deployment is unchanged by this desktop-only compatibility hotfix. | `V1.1.16 RELEASED — LEGACY GENERATED RETEST READY` |
| 2026-07-31 | Vendor Booking v1.1.17 safety/workflow implementation and Windows packaging completed locally. Booking-facing dates use `dd/MMMM/yyyy`; Generate validates the live Service/Supplier/Product/Contract/Rate parent chain before Send while orphan Generated rows can still be returned safely from immutable snapshots. Full Reset Daywise clears Day 0, all Daywise/Micro Split rows, and unsent Generated snapshots, but hard-blocks any customer with Send Attempt or delivery evidence. Rebuild Dates fills blank posted Program headings plus extracted day Start/Finish and arrival/departure fallback times without overwriting manual values. Generated Batch has `Process (N pending)`, continuous next-item processing, compact tree dates, and per-item cancel. Gmail now returns after durable Message/Thread ID storage, displays five persisted stages, and continues official evidence sync in background as `SENT_PENDING_SYNC`. Manager/Admin has a cross-PC Approval Center with maker-checker review and hash-verified Take over sync. The 57-route Post Notification Matrix remains planning-only as explicitly requested. All 65 tests, syntax checks, diff hygiene, ASAR content checks, Windows metadata checks, and the isolated eight-second packaged-app smoke gate pass. Installer size: `111,087,733` bytes. SHA-256: `AB2B548A53006131A2D4C54CED7389BD8F02FD31560D8938E0339AB11F400BC5`. | `V1.1.17 UAT PACKAGE VERIFIED — RELEASE PENDING` |
| 2026-07-31 | Controlled live dummy UAT executed with the persisted packaged-app Google session. Gmail preflight returned `GMAIL_READY` with exact-profile and central-evidence readiness. Dummy booking `ND/PSHBALI7661 / TARANTULA / ATV TANDEM B` produced Gmail Message/Thread ID `19fb6681308934f2`, local Send Attempt `VSEND-6b4e8b1e-0113-4dce-96b2-b49306c9c8ba`, then Retry Sync completed as `SYNCED` with central Communication ID `COMM-2b0646c7-6125-41b2-87e6-bbae73f59b4a`. Gmail readback confirmed the exact Product line, `03/October/2026`, Hotel, Start/Finish, and no Day Wise Header. Parent-integrity UAT also blocked a separate orphan row with `PRODUCT_MISSING`. Cross-PC approval request was safely retained as `LOCAL_PENDING_SYNC`; the live Apps Script returned a generic failure while the local payload/hash/evidence remained durable, so central Approval Center/takeover UAT stays blocked until the Apps Script approval route/schema is redeployed and health-checked. Temporary UAT hooks were removed from source after evidence capture. | `LIVE GMAIL UAT PASSED — APPROVAL BACKEND DEPLOYMENT BLOCKED` |
| 2026-07-31 | Pre-publish rollback backup completed. A consistent SQLite online backup passed `PRAGMA integrity_check` and contains 4 Vendor intake drafts, 20 Vendor bookings, and 6 Send Attempts. Source snapshot, installer, blockmap, `latest.yml`, hashes, and restoration instructions are stored outside the repository under `C:\PROJECT\ERIM-PSH_ROLLBACK\v1.1.17-20260731`. Google Drive folder `1sdFKLt8GrJ0lDP8obCfLQE-H_vZpOUk7` contains a private copy of the Central Data Foundation spreadsheet and all six current itinerary DOCX files; connector readback confirmed seven copied files. OAuth tokens, secure sessions, and client credentials are explicitly excluded. | `ROLLBACK BACKUP VERIFIED` |
| 2026-07-31 | Owner recorded follow-up patch Point 1 for Generate & Send Supplier Booking: every staff-visible and supplier-facing Service date in the editable message, final preview, Service cards, and related communication-workspace labels must display `dd/MMMM/yyyy` with English full month names, while storage, sorting, snapshot data, portal grouping, and online payloads remain ISO `yyyy-MM-dd`. Existing Generated/not-sent snapshot compatibility and immutable Sent history are explicit implementation gates. No source, version, package, deployment, or release was changed. | `FOLLOW-UP PATCH POINT 1 RECORDED — AWAITING IMPLEMENTATION` |
| 2026-07-31 | Owner found two TOC rows (`PANDAWA` and `ULUWATU - KECAK`) trapped in the Generated batch. A read-only audit of the actual local SQLite data confirmed both are unsent snapshots with no delivery attempt, but their original stable Service IDs no longer exist in the active Micro Split source. The current cancel path incorrectly depends on rebuilding through that live source, so an orphan cannot reach its cancellation commit. Follow-up Patch Point 2 records a stored-snapshot-based atomic release, idempotent stale-click handling, and full parent-chain reconciliation across Service, Supplier, Product, and Contract/Rate. Missing, archived, or changed parents keep the immutable Generated row visible as `PARENT CHANGED — REVIEW REQUIRED`, block Send, and expose exact Review/Open archived parent/Cancel/role-controlled Reactivate or Manager override actions. Relinking always cancels and regenerates rather than mutating the old snapshot; Needs Action and Daily Control expose each dependency state. No operational data or source was changed. | `FOLLOW-UP PATCH POINT 2 OWNER-REFINED — ORPHAN/PARENT INTEGRITY GUARD RECORDED` |
| 2026-07-31 | Owner approved the generated-batch spreadsheet design and recorded Follow-up Patch Point 3 for continuous processing. A primary top-header `Process` control starts at the first actionable package in the existing deterministic SOP/channel order and enters the same adaptive delivery workspace used by Review. After each authoritative Email send or external-channel evidence result, staff remains in detail mode and uses prominent Previous/Next pending navigation instead of being returned to the batch list. Back to List preserves context; Review remains an exact-package action; blocked packages stay visible with reasons; zero-pending, resume, cancel-item, and mixed-channel behavior receive explicit UAT. This remains guided manual processing—never bulk or automatic send. No source/version change was started. | `FOLLOW-UP PATCH POINT 3 RECORDED — CONTINUOUS PROCESS MODE` |
| 2026-07-31 | Owner recorded Follow-up Patch Point 4 for the Generate preparation left panel. It must become a genuinely compact Windows Explorer-style `Customer Code → Daywise → Vendor Service` tree with disclosure controls, connector lines, standard typography, compact rows, Customer/Day collapse, exact Product/Supplier leaves, tri-state eligible selection, ancestry-preserving search, and preserved expansion/scroll/selection across refresh. Customer roots sort by Code, Day nodes by Day/date, and Service leaves by the existing stable sort; Generate continues to use stable Service IDs and deterministic regrouping. No source or version change was started. | `FOLLOW-UP PATCH POINT 4 RECORDED — COMPACT PREPARATION TREE` |
| 2026-07-31 | Owner clarified Follow-up Patch Point 5 as an intentional full Daywise restart. After authoritative delivery-ledger validation and explicit OK/Cancel confirmation, Reset removes all local Days including Day 0, all headers/times/details, all Micro Split Services/links, and every Generated draft that has no Send Attempt or delivery evidence. Customer, pax, flight, hotel, Drive/source revision, and itinerary preview remain so staff can run Rebuild Dates from a clean state. Any recorded or ambiguous delivery—Gmail Sent/pending-sync/outcome-unknown/prepared attempt, message/thread ID, or Portal/WhatsApp/Other evidence—blocks the entire reset. The mutation is atomic, audited, idempotent, delists unsent drafts, never reuses removed IDs, and leaves Daily Control at `REBUILD/REPOST REQUIRED`. No implementation was started. | `FOLLOW-UP PATCH POINT 5 OWNER-CLARIFIED — FULL RESET WITH SENT GATE` |
| 2026-07-31 | Owner reported excessive loading after Send Email. Source audit confirmed the foreground path currently performs renderer preflight, a second backend preflight, Gmail send, synchronous Apps Script evidence sync, and a broad application refresh before releasing the UI. Follow-up Patch Point 6 shortens the protected critical path to one final authoritative validation, immutable PREPARED ledger, Gmail send, and durable Message/Thread ID persistence. The UI then acknowledges Gmail acceptance and enables Next pending while idempotent official-evidence sync and broader refresh run in the persistent background. A persistent Supplier-Master-style checklist exposes Gmail connection, snapshot lock, Gmail delivery, local evidence, central sync, and package refresh as Waiting/Running/Done/Failed/Skipped/Blocked with stage-safe Retry or Skip actions. Skip only advances the work session and preserves Needs Action; central-sync retry never resends Gmail, and ambiguous Gmail outcomes require Reconcile or user-controlled resend review. No implementation was started. | `FOLLOW-UP PATCH POINT 6 OWNER-REFINED — VISIBLE SAFE BACKGROUND STAGES` |
| 2026-07-31 | Owner confirmed a cross-PC approval direction for local Supplier Contract/Rate changes. Follow-up Patch Point 7 adds `Manager/Admin → Approval Center`: staff Request Approval uploads the exact normalized payload, before/proposed comparison, evidence reference, maker identity, dependency context, and snapshot hash centrally; Manager reviews it from another PC without accessing staff SQLite. Maker-checker prevents self-approval. Approved and Published remain separate states. The origin PC may resume the exact approved hash after reconnect, or Manager/Admin may perform audited `Take over approved sync` if the origin PC is unavailable. Offline request visibility, version conflicts, duplicate takeover, dependency validation, central readback, compact approval table/diff, and item-level BLOCKED_APPROVAL handling are explicit gates. No implementation was started. | `FOLLOW-UP PATCH POINT 7 RECORDED — CROSS-PC APPROVAL CENTER` |
| 2026-07-31 | Owner requested an official pre-implementation Post Notification record. `docs/POST_NOTIFICATION_MATRIX_PLAN.md` now inventories 57 internal employee-notification routes across Reservation, Vendor, Transport, Ops Accounting, General Cashier, Manager/Admin, and System using the required No/From/To/Purpose/Sample/Status columns. Source audit marks only central New/Revised Itinerary plus Supplier Master initialization/batch/contract-expiry broadcasts Active; every other candidate remains Not Implemented even when a local toast, dashboard state, report, or audit exists. The record defines a future Manager/Admin checklist router, active-route warning, recipient targets, central authority, per-user lifecycle, deep-link/content safety, owner selection process, and implementation gates. No notification routing, recipient, source, deployment, or version was changed. | `OFFICIAL POST NOTIFICATION MATRIX RECORDED — NO IMPLEMENTATION` |
| 2026-07-31 | GC-02 desktop safety patch implementation started after historical Base Plan compatibility review. Implemented code now appends immutable WhatsApp/Portal/Other delivery attempts, supports exact Gmail Sent candidate linking with Message/Thread evidence, removes the OAuth secret value from renderer/bootstrap, applies controlled HTTPS Electron navigation, protects revised Vendor Intake bookings with snapshot lineage plus `AMENDMENT_REQUIRED`, stages itinerary revision posting for safe resume, persists partial Generate batches with retry/skip, and appends `(Itinerary)` to Process Report actions. PWA remains outside this patch. Automated checks and the unpacked packaged build pass; live Gmail/Drive/Apps Script and installed-package UAT remain before release closure. | `IN PROGRESS — CODE PATCHED, RELEASE/UAT PENDING` |
| 2026-07-31 | ERIM-PSH v1.1.18 release candidate completed locally and its backend contract was activated as Apps Script Web App Version 22 on the existing deployment ID/URL. HTTP health readback returns 200 JSON. All 71 automated tests, syntax checks, diff hygiene, packaged ASAR verification, Windows metadata, and an isolated eight-second packaged smoke start pass. The NSIS installer is `111,094,549` bytes with SHA-256 `D8EF0A19619F6A96F6B9CAF6F8EB9018B3E6611455560777C7C9C639DEB935FC`. Git commit/push and GitHub updater release remain before owner installation. | `BACKEND LIVE — GITHUB RELEASE PENDING` |
| 2026-07-31 | ERIM-PSH v1.1.18 GC-02 safety patch released from commit `9689d15`. Branch `agent/desktop-v1` is pushed; GitHub Release `v1.1.18` is public and marked Latest with installer, blockmap, and `latest.yml`. GitHub's online installer digest matches local SHA-256 `D8EF0A19619F6A96F6B9CAF6F8EB9018B3E6611455560777C7C9C639DEB935FC`. Apps Script Web App Version 22 remains active and healthy on the existing URL. Controlled Gmail evidence-linking, staged Drive-revision recovery, and operator workflow UAT remain; PWA is unchanged and stays in its separate milestone. | `RELEASED — CONTROLLED LIVE UAT` |
| 2026-07-30 | A source-backed local end-to-end audit of Vendor Generate and Email was recorded as Section 12 of `docs/VENDOR_V119_UAT_FOLLOWUP_NOTES.md`. The simulation executed the real SQLite, preview/generate, immutable Send Attempt, MIME construction, Gmail-acceptance, Apps Script evidence, resend, and evidence-retry paths with mocked external responses. It proved Gmail Message/Thread ID persistence, double-send blocking, linked resend attempts, and `SENT_PENDING_SYNC` retry without another Gmail call. The follow-up checklist records automatic Gmail ID marks, the currently incomplete post-popup Resend positioning, Delivery History entry points, all-attempt thread scanning, original-thread reply preservation, backend TO validation before ledger insertion, field inclusion decisions, implementation order, and UAT gates. No production email, Apps Script mutation, or application-source change was performed. | `AUDITED — FOLLOW-UP RECORDED` |
| 2026-07-30 | The local Supplier Rate approval and sync SOP was approved and recorded in `docs/SUPPLIER_LOCAL_RATE_APPROVAL_SYNC_SOP.md`. A complete local Contract/Rate may remain immediately usable on the originating PC while visibly marked `LOCAL RATE — SYNC APPROVAL REQUIRED`; local operational usability is separated from approval/publication state. The SOP defines required source/evidence, local/central visibility, Request Approval notice, maker-checker roles, approved snapshot hash, dependency-ordered sync, rejection/conflict impact on Generated and Sent bookings, refresh overlay preservation, booking provenance, reminders, 16 implementation items, and 18 UAT gates. Current local-draft overlay already makes valid local rates visible in the local catalog, but approval-specific state enforcement and UI notices are not yet implemented. | `SOP RECORDED — IMPLEMENTATION REQUIRED` |
| 2026-07-30 | Owner approved deterministic Vendor Generate sorting and an early Gmail connection preflight. Packages must sort by channel/SOP priority, Supplier, Customer Code, first service date, first Day, then stable Package Key; services inside a package must sort by Day, service date, Split sequence, Product, then stable Service ID. Selected Email packages must refresh/validate the OAuth token and verify Gmail `users/me/profile` at Generate, with Reconnect, Generate Draft Only, or Cancel on failure. A sticky readiness card shows verified sender, check time, session state, and separate central-evidence readiness. Gmail connection, account, TO recipient, and unchanged snapshot are checked again immediately before Send Attempt creation because a Generate-time check is only early warning. The complete states, implementation order, and 22 UAT gates are recorded in Section 13 of `docs/VENDOR_V119_UAT_FOLLOWUP_NOTES.md`. | `APPROVED — RECORDED FOR IMPLEMENTATION` |
| 2026-07-30 | Portal sorting and transaction grouping were added to the approved Priority 1 Generate rules. Global channel priority remains Email, WhatsApp, Portal, Others, but Portal packages sort by Customer Code, Supplier, first service date, Product, and stable key. Same-customer outbound and return services may share one Portal ticket reference only when the stable Supplier is also the same and the supplier issues one transaction; different outbound/return suppliers always create separate packages and references. Each package retains its exact stable Service IDs, and a possible omitted same-supplier return produces a non-blocking notice without automatic selection. | `APPROVED — RECORDED FOR IMPLEMENTATION` |
| 2026-07-30 | A supplier-specific Portal payment notice was approved for the configured stable Eka Jaya identity. Calendar-day lead time is measured from the Portal booking/action date to the earliest included service date, so a round trip uses its outbound date. Exactly 15 days or less displays `USE DEPOSIT`; more than 15 days displays `USE PREPAID`. The Portal panel must recheck the rule before opening the supplier site and preserve the calculation inputs, result, and rule version in booking evidence. Boundary UAT covers 14, 15, and 16 days. | `APPROVED — RECORDED FOR IMPLEMENTATION` |
